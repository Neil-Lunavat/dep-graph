...
| D23 | Embedding model | **Decided (delegated)**, revised before any run | `jinaai/jina-embeddings-v2-base-code`, revision `516f4baf13dec4ddddda8631e019b5737c8bc250`, **first 512 tokens** of each file, L2-normalised, cosine similarity; run on the rented server's CPU, never on Neil's laptop GPU (in use) | 2026-09-22 | First choice was the full 8,192-token window; measured 0.28 files/s on CPU = ~120 h for 124,767 unique file versions. 512 tokens measured 3.96 files/s = ~9 h. Changed before any method ran |
