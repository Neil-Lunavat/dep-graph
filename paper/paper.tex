...
combination, on 2{,}412 change tasks from 1{,}162 real pull requests in 112 Python
...
Table~\ref{tab:priorkeys} records it for the four systems we could check directly. Where
localization accuracy is reported at all, the target is the set of files or functions
...
weaker of the two signals. \TODO{extend this to 20--30 papers; four verified entries are not
a survey, and the claim that the patch-based key is the field default should rest on a
systematic sample}

\begin{table}
\caption{What graph-based localization systems are graded against. Verified from the
published papers on 2026-09-23.}
\label{tab:priorkeys}
\small
...
\midrule
LocAgent~\cite{chen2025locagent} & ACL 2025 & Files and functions modified by the patch, on
SWE-bench-Lite; Acc@$k$ over the top-$k$ predicted locations \\
CoSIL~\cite{cosil2025} & ASE 2025 & File- and function-level localization accuracy on
SWE-bench Lite and Verified, i.e.\ the patched locations \\
Agentless~\cite{xia2024agentless} & FSE 2025 & Localization then repair on SWE-bench Lite;
localization is scored against the patched files \\
RepoGraph~\cite{ouyang2025repograph} & ICLR 2025 & Not localization accuracy: a plug-in
scored by end-to-end issue resolution rate \\
\bottomrule
\end{tabular}
\end{table}
...
design, the claims and the text. Bibliographic entries were drafted with AI assistance; the
seven entries for recent systems and benchmarks were checked online against arXiv, the ACL
Anthology and the publisher listings on 2026-09-23, and the entries in
Table~\ref{tab:priorkeys} were read from the papers themselves. \TODO{the remaining
entries, all older and written from memory, still need checking against the published
record.}
